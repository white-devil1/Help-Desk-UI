...
const api = axios.create({
  baseURL: import.meta.env.VITE_API_URL || `http://${window.location.hostname}:8002`,
  headers: {
    'Content-Type': 'application/json',
  },
});
...
// ✅ REAL GENERATIVE AI - Calls your trained transformer model
export const callCustomAssetAI = async (message, userId = 1, companyId = 1) => {
  try {
    console.log('🤖 Calling REAL Generative LLM at http://localhost:8003/v1/chat');
    ...
    const data = await response.json();
    console.log('✅ REAL Generative AI Response:', data);
    ...
    console.error('❌ REAL LLM call failed:', error);
    
    // Show error to user
    return {
      response: "⚠️ AI service is starting up. Please wait a moment and try again.",
      action: "error",
      confidence: 0.0,
      model_type: "fallback"
    };
